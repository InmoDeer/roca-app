...
const SUPABASE_ANON_KEY =
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Ind2aWhoZ2h1b2F5cnJ0ZG1lbWZvIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzU0NDU2NDIsImV4cCI6MjA5MTAyMTY0Mn0.86rstgilTvVZgV5KRNPmb7oBx8Xa73e39Sd62_OmkVI';
...
function buildOutputs(p) {
  ...
  const precio = `💰 ${p.operacion}: ${sym}${Number(
    p.precio
  )?.toLocaleString()}`;
  ...
  const caracteristicas = [
    p.dormitorios
      ? `🛏 ${p.dormitorios} ${
          p.dormitorios == 1 ? 'dormitorio' : 'dormitorios'
        }`
      : '',
    p.ambientes
  ...
  return (
    <div style={S.galleryOverlay} onClick={onClose}>
      <div style={S.galleryBox} onClick={(e) => e.stopPropagation()}>
        <button onClick={onClose} style={S.galleryClose}>
          ✕
        </button>
        <img src={fotos[idx]} alt="" style={S.galleryImg} />
        <div style={S.galleryCount}>
          {idx + 1} / {fotos.length}
        </div>
        {fotos.length > 1 && (
        ...
        <div style={S.galleryThumbs}>
          {fotos.map((f, i) => (
            <img
              ...
              style={{
                ...S.galleryThumb,
                outline: i === idx ? '2px solid #e8ff4f' : 'none',
              }}
            />
          ))}
        </div>
        ...
        <div style={S.modalBody}>
          ...
          <div style={S.row2}>
            <Sel
              ...
              form={form}
              setForm={setForm}
              opts={['Departamento', 'Casa', 'Local', 'Oficina', 'Terreno']}
            />
            <Sel
              label="Operación*"
              k="operacion"
            ...
            <div style={S.photoGrid}>
              {form.fotos_urls.map((url, i) => (
                <div key={i} style={S.photoThumbWrap}>
                  ...
                  <button onClick={() => removePhoto(i)} style={S.photoRemove}>
                    ✕
                  </button>
                </div>
              ))}
    ...
    <div style={S.detail}>
      ...
      {out.fotos.length > 0 && (
        <div style={S.heroWrap} onClick={() => setGallery(true)}>
          ...
          {out.fotos.length > 1 && (
            <div style={S.heroBadge}>📸 {out.fotos.length} fotos</div>
          )}
        </div>
      )}
      ...
      <div style={S.msgBox}>
        ...
        <CopyShareBtns
          text={tab === 'corto' ? out.mensajeCorto : out.mensajeLargo}
        />
      </div>
    ...
    () =>
      properties.filter((p) => {
        ...
        if (
          ...
          return false;
        if (filters.operacion && p.operacion !== filters.operacion)
          return false;
    ...
    <div style={S.app} onClick={() => setOpenMenu(null)}>
      ...
      <div style={S.filterRow}>
        {[
          ...
          {
            k: 'estado',
            opts: ['', 'Disponible', 'Reservado', 'Vendido/Alquilado'],
            label: 'Estado',
          },
        ].map(({ k, opts, label }) => (
          <select
        ...
        {filtered.map((p) => {
          ...
          return (
            <div key={p.id} style={S.card} onClick={() => setSelected(p)}>
              {out.fotos.length > 0 && (
                <img src={out.fotos[0]} alt="" style={S.cardThumb} />
              )}
