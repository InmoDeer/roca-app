...
export function RocaTabs({ tabs = [], defaultValue, onValueChange }: any) {
  ...
  const triggerStyle = (isActive: boolean): any => ({
    flex: 1,
    padding: "10px 16px",
...
export function SimpleTabs({ tabs = [], value, onValueChange, variant = "pill" }: any) {
  const { t, mode } = useTheme();
  ...
  const tabStyle = (isActive: boolean) => variant === "pill" ? {
    flex: 1,
    padding: "10px 16px",
    borderRadius: t.radius.sm,
    border: "none",
